...
docker run -d --gpus 1 --shm-size 8G --name backbone_trainer_pytorch --mount type=bind,source="$(pwd)"/save,destination=/code/save --mount type=bind,source=$HOME/transfer,destination=/code/transfer backbone_pytorch